...
const ContactForm = ({ email, budget }) => {
  ...
  const emailBuffer = Buffer.from(email, 'base64')
  const decodedEmail = emailBuffer.toString('utf-8')

  const [target, setTarget] = useState(`https://formspree.io/${email}`);
        ...
        <div className="flex flex-wrap w-full">
          <div className="w-full lg:w-1/3 py-px lg:px-px">
            <input
              ...
              required
              type="text"
              onChange={e => setTarget(`https://formspree.io/${decodedEmail}`)}
            />
          </div>
          <div className="w-full lg:w-1/3 py-px lg:px-px">
            <input
